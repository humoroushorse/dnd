...
def test_get(client: TestClient) -> None:
    ...
    response_schema = schemas.responses.GenericListResponse[schemas.dnd_class.DndClassBase](**response.json())
    assert response.status_code == status.HTTP_200_OK
...
def test_delete(client: TestClient, random_dnd_class: schemas.dnd_class.DndClassCreate) -> None:
    """Test delete: happy path."""
    response = client.delete(f"{uncached_settings.API_V1_STR}/classes?id={random_dnd_class.id}")
