...
class JtSpellToClass(DbBase):
    ...
    spell_id: Mapped[int] = mapped_column(ForeignKey(f"{schemas.enums.DbSchemaEnum.DND.value}.spell.id"))
    dnd_class_id: Mapped[int] = mapped_column(ForeignKey(f"{schemas.enums.DbSchemaEnum.DND.value}.dnd_class.id"))
    # relationships
